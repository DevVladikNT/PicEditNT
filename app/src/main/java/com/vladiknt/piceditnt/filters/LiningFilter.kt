...
object LiningFilter {
    ...
    private fun render() {
        ...
        for (i in 0 until image.width) {
            for (j in 0 until image.height) {
                val pixel = image[i, j]
                val red = Color.red(pixel).toDouble() + 0.3 * (average - matrix[j][i])
                val green = Color.green(pixel).toDouble() + 0.3 * (average - matrix[j][i])
                val blue = Color.blue(pixel).toDouble() + 0.3 * (average - matrix[j][i])
