...
/**
 * Фильтр, выделяющий основные оттенки в областях изображения.
...
object NeonFilter {
    ...
    private fun render() {
        ...
        for (i in 0 until image.width) {
            for (j in 0 until image.height) {
                val pixel = image[i, j]
                val red = Color.red(pixel).toDouble() * (average / matrix[j][i])
                val green = Color.green(pixel).toDouble() * (average / matrix[j][i])
                val blue = Color.blue(pixel).toDouble() * (average / matrix[j][i])
